...
export const analyzeSingleIssueWithGemini = async (issue: GithubIssue): Promise<{ features: FeatureRequirement[], decisions: DesignDecision[] }> => {
  try {
    ...
    if (!text) {
      console.warn(`Issue #${issue.number}: Gemini returned an empty response (possibly blocked by safety filters).`);
      return { features: [], decisions: [] };
    }
    return JSON.parse(text);
  } catch (e) {
    console.error(`Error in analyzeSingleIssueWithGemini for #${issue.number}:`, e);
    return { features: [], decisions: [] };
  }
};
